...
*       - force source-level 4-FMA accumulation (explicit fmaf)
* */
    ...
    for(SubApertureImage& sub : subapertures){
        ...
        for (int y = y_begin; y < y_end; ++y) {
            ...
            for (int x = x_begin; x < x_end; ++x) {
                ...
                // Force the 4-FMA accumulator form: fold the running sum in as
                // the accumulator so each term is a single fmaf.  The three RGB
                // chains stay independent for ILP.
                float accr = vals[idx*3];
                float accg = vals[idx*3 + 1];
                float accb = vals[idx*3 + 2];

                accr = __builtin_fmaf(A, pTLr, accr);
                accg = __builtin_fmaf(A, pTLg, accg);
                accb = __builtin_fmaf(A, pTLb, accb);

                accr = __builtin_fmaf(B, pTRr, accr);
                accg = __builtin_fmaf(B, pTRg, accg);
                accb = __builtin_fmaf(B, pTRb, accb);

                accr = __builtin_fmaf(C, pBLr, accr);
                accg = __builtin_fmaf(C, pBLg, accg);
                accb = __builtin_fmaf(C, pBLb, accb);

                accr = __builtin_fmaf(D, pBRr, accr);
                accg = __builtin_fmaf(D, pBRg, accg);
                accb = __builtin_fmaf(D, pBRb, accb);
