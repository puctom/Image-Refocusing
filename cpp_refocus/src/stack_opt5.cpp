...

    {
        ...
        for (size_t f = 0; f < NF; ++f) {
            ...
            for (int y = 0; y < h; ++y) {
                ...
                for (int x = 0; x < w; ++x) {
                    row += diff[y * (W + 1) + x];
                    counts[f][y * w + x] = static_cast<int16_t>(
                        row + (y > 0 ? counts[f][(y - 1) * w + x] : 0));
                }
            }
        }
    }
    ...
    for (size_t f = 0; f < NF; ++f) {
        ...
        for (size_t y = 0; y < H; ++y) {
            for (size_t x = 0; x < W; ++x) {
                ...
                if (c == 0) continue;
                out.at(x, y, 0) = (unsigned char) std::clamp(
                    std::round(vals[f][idx * 3]     / c), 0.0f, 255.0f);
                out.at(x, y, 1) = (unsigned char) std::clamp(
                    std::round(vals[f][idx * 3 + 1] / c), 0.0f, 255.0f);
                out.at(x, y, 2) = (unsigned char) std::clamp(
                    std::round(vals[f][idx * 3 + 2] / c), 0.0f, 255.0f);
            }
        }
