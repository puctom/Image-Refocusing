        ...
        for(int y=y_begin; y < y_end; ++y){
            for(int x=x_begin; x < x_end; ++x){
                ...
                for(int c=0; c<3; ++c){
                    ...
                    const float p00 = sub.data.data[((y + sy)     * width + x + sx)     * 3 + c];
                    const float p10 = sub.data.data[((y + sy)     * width + x + sx +1)  * 3 + c];
                    const float p01 = sub.data.data[((y + sy + 1) * width + x + sx)     * 3 + c];
                    const float p11 = sub.data.data[((y + sy + 1) * width + x + sx + 1) * 3 + c];

                    // Force the 4-FMA accumulator form: fold the running sum in
                    // as the accumulator so each term is a single fmaf.
                    float acc = vals[idx*3 + c];
                    acc = __builtin_fmaf(A, p00, acc);
                    acc = __builtin_fmaf(B, p10, acc);
                    acc = __builtin_fmaf(C, p01, acc);
                    acc = __builtin_fmaf(D, p11, acc);
                    vals[idx*3 + c] = acc;
                }
