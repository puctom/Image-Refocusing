...
ImageData refocus_shift_and_sum(std::vector<SubApertureImage>& subapertures, float focus) {
    ...
    for (const auto& p : params) {
        for (int y = p.y_begin; y < p.y_end; ++y) {
            int x = p.x_begin;
            for (; x + 8 <= p.x_end; x += 8) {
                __m256i c = _mm256_loadu_si256((__m256i*)&counts[y * width + x]);
                c = _mm256_add_epi32(c, _mm256_set1_epi32(1));
                _mm256_storeu_si256((__m256i*)&counts[y * width + x], c);
            }
            for (; x < p.x_end; ++x) counts[y * width + x]++;
        }
    }
