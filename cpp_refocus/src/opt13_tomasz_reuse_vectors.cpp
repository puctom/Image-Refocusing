    ...
    for (int ty = 0; ty < h; ty += TILE_H) {
        ...
        for (int tx = 0; tx < w; tx += TILE_W) {
            ...
            for (const SubParams& p : params) {
                ...
                const int x_start = (x_begin - tx) * 3;
                ...
                for (; x + 8 <= x_stop; x += 8) {
                    ...
                    size_t ind_ltop = ((y_begin + p.sy) * width + (x_begin + p.sx)) * 3 + x_offset;
                    ...
                    size_t ind_rbot = ind_rtop + width * 3;
                    // Main loop
                    for (int y = y_begin; y < y_end; ++y) {

                        // tile_vals row pointer for output row y, starting at column x_begin
                        float* vp = tile_vals.data() + (y - ty) * tile_w_actual * 3 + x;

                        // Load bottom registers
                ...
                if (remaining > 0) {
                    int x_offset = x - x_start;
                    for (int y = y_begin; y < y_end; ++y) {
                        // Recalculate indices for the remaining edge cases
                        size_t ind_ltop = ((y + p.sy) * width + (x_begin + p.sx)) * 3 + x_offset;
                        size_t ind_lbot = ind_ltop + width * 3;
                        size_t ind_rtop = ind_ltop + 3;
