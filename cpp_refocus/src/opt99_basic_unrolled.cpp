        ...
        for (int tx = 0; tx < w; tx += TILE_W) {
            ...
            for (const SubParams& p : params) {
                ...
                for (int y = y_begin; y < y_end; ++y) {
                    ...
                    // Unrolled main loop: process 32 consecutive channel bytes.
                    //
                    // This is two full YMM accumulator vectors per iteration.
                    // It gives the CPU more independent work and reduces loop overhead.
                    for (; x + 32 <= x_stop; x += 32) {
                        __m128i ltop_b0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_ltop));
                        __m128i rtop_b0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_rtop));
                        __m128i lbot_b0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_lbot));
                        __m128i rbot_b0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_rbot));

                        __m128i ltop_b1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_ltop + 16));
                        __m128i rtop_b1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_rtop + 16));
                        __m128i lbot_b1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_lbot + 16));
                        __m128i rbot_b1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(p.SUB + ind_rbot + 16));

                        __m256i add0 = weighted_half_contrib_16_avx2(
                            ltop_b0, rtop_b0, lbot_b0, rbot_b0, coeff_ab, coeff_cd
                        );

                        __m256i add1 = weighted_half_contrib_16_avx2(
                            ltop_b1, rtop_b1, lbot_b1, rbot_b1, coeff_ab, coeff_cd
                        );

                        __m256i acc0 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(vp));
                        __m256i acc1 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(vp + 16));

                        acc0 = _mm256_add_epi16(acc0, add0);
                        acc1 = _mm256_add_epi16(acc1, add1);

                        _mm256_storeu_si256(reinterpret_cast<__m256i*>(vp), acc0);
                        _mm256_storeu_si256(reinterpret_cast<__m256i*>(vp + 16), acc1);
                    ...
                    for (; x + 16 <= x_stop; x += 16) {
                        ...
                        __m256i addv = weighted_half_contrib_16_avx2(ltop_b, rtop_b, lbot_b, rbot_b, coeff_ab, coeff_cd);
