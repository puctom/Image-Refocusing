...
Figure \ref{fig:avxCyclesPerPixelResults} plots the performance for the AVX2 versions of the code. Our fully optimized vectorized code brings the runtime from the previous best scalar $13.9$~Gcycles down to $2.0$~Gcycles, a further $7.0\times$ speedup, which is a total $62.5\times$ speedup over the original baseline. The best vectorized single focus code reaches ... \todo flops/cycle, ...\% of the scalar peak. The speedup is close to but slightly below the expected $8\times$ from processing 8 single-precision floats per AVX2 register. The IPC drops from 1.18 to 0.78 because byte-to-float conversions dispatch to the same two ports as the FMA instructions. Since each loop iteration issues an equal number of conversion and FMA instructions, the sustained throughput is bounded at roughly half the SIMD peak --- this is why we cannot reach the SIMD roof.
