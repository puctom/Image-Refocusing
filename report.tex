...
Figure \ref{fig:avxCyclesPerPixelResults} plots the performance for the AVX2 versions of the code. AVX2 brings the runtime from the previous best scalar \todo{check deez numbers} $11.5$~Gcycles down to $1.97$~Gcycles, a further $5.84\times$ speedup, which is a total $44.16\times$ speedup over the original baseline. Next we present the roofline model and the measured FLOPs/cycle over the execution.
...
\todo{Replace with flops/cycle graph}
\begin{figure}[H]
    \centering
...
Similarly, as for the scalar code, our roofline model contains different performance roofs depending on the instruction mix and conversions reuse. Our most optimized code reaches ... \todo flops/cycle, ...\% of the scalar peak for its corresponding instruction mix. 

The increased computation capabilities of the vectorized code implemented with vector intrinsics are accompanied by tackling the data transfer stalls. The cache blocking with arbitrary and optimized tile size reduce the cache misses across different levels of cache, while the loads and conversion reuse further reduce the performed accesses to L1 cache, since the data stays in the registers. 
...
\todo{improve the colors in this plot, will do tonight, must go}
