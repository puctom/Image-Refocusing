...
\subsection{Vectorized Single-Focus Optimizations}\label{subsec:vectorized-single-focus}
...
\mypar{Best compiler vectorized code} Before hand-crafting the vectorized code, we compiled our final, most-performant scalar code without the \texttt{-fno-tree-vectorize} flag. This gave us the compiler-vectorized code, which we evaluated in the further experiments. There was no significant performance difference compared to the scalar code (less than 1\% difference).

Our brief inspection of the produced assembly showed that the compiler failed to vectorize the global algorithm structure. Instead, it only vectorized a single iteration of the innermost loop, heavily underutilizing the vectorization capabilities.

\mypar{Initial vectorized code with intrinsics}
Looking at the algorithm globally, it became clear that for a given subaperture image, adjacent output pixels and their channels can be processed in parallel: they all accumulate contributions from the neighbouring subaperture data, accessed in the same sequential pattern. For these reasons we opted for explicit AVX2 intrinsics in the implementation.
...
For small image sizes (below 1024 pixels) this reduces runtime by 77\%; for larger images the gain is only about 61\%. Our investigation showed that this 40\% worse performance at larger sizes occurs because the output accumulator array does not fit into the cache, which motivates the next optimization.


\mypar{Blocking for cache hits on output image}
With the established \texttt{sub-y-x} loop order, the whole output image is accessed over and over again for every subaperture. However, the cache levels (as summarized in Table \ref{tab:cache_specs}) are too small to fit the output image (and the currently processed subaperture) in their entirety, which results in misses across all levels when accessing the output image, in turn causing the execution to stall. Already at $1024\times 1024$ pixels, the output accumulator array requires $1024 \cdot 1024 \cdot 3 \cdot 4\,\text{B} = 12\,\text{MB}$, which exceeds the 8\,MB LLC capacity.


Bearing in mind the regular, sequential structure of the pixel accesses, we decided to process a smaller tile of the output image for all subapertures at a time, ensuring that when accessed for subsequent subapertures that output memory stays resident in the cache rather than being evicted due to its total size. An arbitrary tile size is possible, with the access pattern illustrated in Figure~\ref{fig:cacheBlocking}, achieved by using \texttt{tile\_y - tile\_x - sub - y - x} loop order.
...
\begin{figure}[H]
    ...
    \caption{Blocking the output image. One output tile is kept cache-resident while all $n$ subapertures stream through it, each contributing only its overlapping region. Shaded out tiles mark regions that have already been processed.}
...
We then tuned the tile dimensions empirically. Rather than scanning the height-width grid exhaustively, we used a Bayesian optimizer over the two parameters, which iteratively proposes tile sizes and converges on the combination that minimizes the median measured cycle count. We utilized the Weight and Biases platform  \cite{wandb} to run these simulations and kept a fixed image size of 2048x2048 when running them. The best results were achieved by the tile sizes with large width (around 2000) and small height (around 8), as shown in Figure~\ref{fig:wandb}. Ultimately, a tile size of 8 rows $\times$ 2048 columns (one full row) was chosen.

This was initially surprising, since the chosen tile exceeds the L1D cache, violating our original residency hypothesis. However, with a full-width tile, each subaperture's contributing region is traversed as complete contiguous rows, which maximizes their reuse from the lower cache levels; a narrower tile would fragment each row into short segments and lose that reuse. Additionally, since the algorithm is compute-bound, some of the latency of the L1 misses can be hidden behind the compute operations, which saturate the FMA and conversion ports and thus dominate the runtime. Indeed, the L1D miss rate rises from roughly 6\% for the smaller tiles to about 12\% for the larger ones, yet performance still improves, confirming that L1 residency of the output was not the binding factor. 
...
Taken together, these optimizations cut the LLC absolute load count from 704k to 363k at the 2048×2048 image size, with the corresponding miss ratio falling from roughly 85–95\% to 60–70\% across various image sizes.


\mypar{Increasing Instruction-Level Parallelism}
On top of the code with cache-oriented tiling, we explicitly introduced two independent AVX accumulation chains in the loop body, processing 16 output channels per iteration instead of 8. This way, the compiler explicitly generates two groups of independent conversions (\texttt{vpmovzxbd}, \texttt{vcvtdq2ps}) and FMA (\texttt{vfmadd132ps}) instructions in the assembly. This gave, however, only about 4\% speedup. Our analysis concluded that the bottleneck is rather the port saturation (conversions and FMA both dispatch to the same two ports), hence explicitly adding a second independent accumulation chain does not relax this limitation and only has a marginal gain. 


\mypar{Load and conversion reuse} %todo add ref to scalar version - cannot because subsubsections are banned
The final optimization with the vectorized code followed the same idea as the \textit{Load and conversion reuse} for scalar code. As illustrated in Figure~\ref{fig:avxReuseIllustration}, in the case of the vectorized code however, the contributing subaperture pixels cannot be reused for row-adjacent output vectors because the output channel stride is 8 while the subaperture channel stride is 3. Fortunately, the intermediate rows of the tile can be used first as the bottom corners of one output row and then as the top corners of the row below.
By processing row-by-row within each tile, we reuse these intermediate loaded and converted rows directly from registers. This was applied in blocks of 4 rows, leading to reduction of the loads and conversions from 8 (2 per row) to 5 (reusing 3 intermediate rows). The aim was for the loaded and converted data to be used again right away from the registers, rather than to need to be fetched from the cache again.  This optimization reduces the total FLOPs performed and improves runtime by a further 25\%, while FLOPs/cycle stay roughly the same (the code is still bottlenecked at the same two ports).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{ASL Project-Page-17.drawio-4.pdf}
    \caption{AVX implementation and reuse of loads and conversions. The highlighted inner input-row of subaperture is used for both the output row above and below, our optimization loads and converts it only once.}
...
\mypar{Single-focus scalar results}
\begin{figure}[H]
    \centering
...
\end{figure}
 Figure \ref{fig:scalar-perf} plots our single-focus runtime against image size for the scalar optimizations. The loop reordering, inlining, ILP, conversion reuse, and prefix sum count calculation add up to an $7.56\times$ speedup over the baseline, from $87$ down to $11.5$~Gcycles for inputs of size 2048. The performance drops heavily for images of size 3072x3072 and above. This is because the 289 subaperture images take up more than 8GB of space for these image sizes, meaning that these images no longer fit into our machine's RAM. This forces the program to use the swap space for storing subaperture images, which incurs disk I/O latency and throughput penalties when accessing the data. Because of this, the rest of our discussion in Section 4 will be focused on images of size 2048x2048 and below. 

The scalar peak performance of our processor is 4 FLOPs/cycle through 2 FMAs across 2 ports. However, as explained in Section 2, our algorithm also requires int-to-float conversions. The conversion operation has a peak performance of 2 FLOPs/cycle, and since we are performing half as many conversions as FMAs after the Conversion Reuse optimization, we have an Instruction Mix Peak Performance of 3.33 FLOPs/cycle.

We see from Figure \ref{fig:scalar-perf} that our most optimized scalar code performance hovers around 3.2-3.3 FLOPs/cycle. This is very close to the Instruction Mix Peak Performance, meaning that there is very little room for improvement. Since the per-pixel workload is constant, total flops and runtime both scale linearly with the number of pixels, so measured performance is effectively independent of image size.
...
Our roofline plot in Figure \ref{fig:scalar-roofline} was generated with help from Intel® Advisor MLR model. The raw results were altered to include int-to-float conversions as FLOPs. On the plot, \textbf{baseline} and \textbf{tiling} code versions closely overlap for all of their input sizes, while the \textbf{DP counts} and \textbf{conv. reuse} versions overlap almost exactly for images of sizes 1024 and 2048, hence the seemingly missing orange square and rhombus in Figure \ref{fig:scalar-roofline}.

The plot reveals a few important insights. First of all, we notice that all versions of the code, including the baseline, are compute bound. Next, we see that the baseline version has a much higher operational intensity compared to the optimized versions. The reason for this is twofold; first, the baseline does not reuse weights across subapertures, so it performs many more unnecessary operations, roughly 1.8x as many in our measurements. Second, the access pattern of the baseline implementation is more friendly for the DRAM, but much-less friendly for L1 cache, which is not apparent from the DRAM-level roofline plot.

Next, we can see that for our optimized versions, increasing the image size consistently decreases operational intensity while achieving the same performance. This is because our scalar code accesses the entire output image accumulator for each subaperture. As the output image gets larger, the accumulator stops fitting into LLC and we end up getting more cache misses and causing more memory traffic. This is confirmed by the operational intensity of the \textbf{tiling} code version in Figure \ref{fig:scalar-roofline}, where we tiled the output image. This optimization does not provide any speedup and was therefore omitted for scalar code, but we can see that it has a higher operational intensity than other optimized code versions even for larger image sizes. 


\mypar{Vectorized Single-Focus Optimizations Results} \\
Our AVX2 optimizations bring the runtime from the previous best scalar $11.5$~Gcycles down to $1.97$~Gcycles, a further $5.84\times$ speedup, which makes it a total $44.16\times$ speedup over the original baseline. 

As described in Section~\ref{subsec:vectorized-single-focus}, the increased SIMD throughput of the intrinsics-based implementation was paired with tackling the data transfer stalls. Cache blocking reduced misses across all cache levels, while load-and-conversion reuse approach kept the fetched data in registers, reducing L1D traffic. Besides the anticipated runtime improvement, in the experiments the Linux \texttt{perf} hardware counters confirmed that, as expected, the number of L1D cache loads decreases by approximately 30\% across all image sizes.
% (FIGURE REMOVED) as depicted in Figure~\ref{fig:avxReuseL1DLoads}.

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=1\linewidth]{l1d_loads_normalized.pdf}
%     \caption{L1D cache loads normalized to the no-reuse version. In the most optimized code the fetched-and-converted data stays in registers.}
%     \label{fig:avxReuseL1DLoads}
% \end{figure}

% Yeah, I guess not so important to have this plot

With all these optimizations, the most optimized code reaches approximately 18.5 \todo{Check these numbers, no access to the data for it, @Klejdi help if possible} FLOPs/cycle, which is around 70\% of the theoretical peak for this instruction mix (26.67 FLOPs/cycle), see the performance plot \ref{fig:avxPerformancePlotFlops}. Similarly, as with the scalar code, the roofline plot in Figure~\ref{fig:avxRooflinePlot} shows different performance roofs depending on the instruction mix and the conversion reuse. The plot additionally includes the performance of the most optimized focal stack implementation, discussed more in detail further below.
% (Tomasz - I commented out, I don't think it's relevant) The IPC drops from 1.18 to 0.78 because the byte to float conversions use the same ports for the vector instructions as the vector FMAs. This is also why we cannot reach the SIMD roof.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{avx_perf_plot.png}
    \caption{Performance Plot for Vectorized Optimizations}
    \label{fig:avxPerformancePlotFlops}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{report/figures/roofline_ilp_avx.pdf}
    \caption{Roofline Plot for Vectorized Optimizations}
    \label{fig:avxRooflinePlot}
\end{figure}
\todo{JUST FOR SCALE, THAT'S NOT THE AVX ROOFLINE, STILL WORKING ON IT}



The code does not reach the theoretical SIMD peak (26.67 FLOPs/cycle) because even with the aforedescried optimizations, there are still memory access penalties incurred (e.g. from the conflict misses etc.), preventing the full utilization of the ports.
\todo{important question that Jonas also asked.}

\mypar{Focal Stack Optimization Results}
The scalar optimizations bring the stack baseline from 1772.4 down to 264.7 Gcycles, which is a $6.7\times$ gain. For 21 focal values and images of size $2048\times2048$, the initial AVX2 speedup over the best scalar stack version was around $3.9\times$. Focus blocking bring this to 41.7 Gcycles, which in total results in a $42.5\times$ speedup over the stack baseline. As the Figure~\ref{fig:avxCyclesPerPixelResults} shows, the best stack code reaches \todo{} flops/cycle, ..\% of the SIMD peak, which is almost the same as our best single-focus code so with a whole stack the work required per pixel is similar to as with one refocused image. For images of size $2048\times2048$, the single focus code called over all focal values actually outperforms our stack version by around 20\% in total cycles, due to the stack optimized code keeping the count maps and output images for all F focuses in memory at once, which exceeds the 8GB of RAM available on our processor. For the 200 focus values, the Linux \texttt{perf} tool measured $2.4\times10^5$ major page faults, as opposed to the single-focus version which has none, which explains the slowdown. \todo{I moved this bit from the optimizations to the results but i think it makes more sense if we have a plot to show this, otherwise im not sure if we should keep it but it is good for analysis}
...
In this project we optimized a single-core C++ light-field refocusing reference baseline implementation for a target Intel Tiger Lake microarchitecture. The optimizations were guided by roofline analysis, hardware performance counters and code analysis. The following optimizations were performed in 3 stages. Scalar optimizations -- including loop reordering, function inlining, load and conversion reuse, and a prefix-sum count algorithm --- gave a $7.56\times$ speedup over the baseline, and nearly achieving its instruction-mix ceiling of 3.33 FLOPs/cycle.

The vectorized implementation was hand-crafted use of AVX2 intrinsics with cache blocking and cross-row load-and-conversion reuse, which gave a further $5.84\times$ speedup, a total of $44\times$ over the original baseline. The best vectorized code reaches approximately 70\% of the corresponding AVX2 instruction-mix ceiling of 26.67 FLOPs/cycle. Lastly, the focal-stack optimizations focused on cache-friendly accesses to reused subaperture images, by adding an extra blocking of the input focus lengths. This achieved very similar performance as the single-focus code.

Further work exploring new or the abandoned ideas, as well as further extension to exploit multi-core parallelism and targeting different machines is encouraged. 
...
\mypar{Tomasz} Implemented early focal-stack optimizations, isolated AVX optimizations for analysis of specific approaches, implemented explicit increasing of ILP in the AVX code. Worked on the infrastructure for hardware counters and stable measurements and provided insightful metrics for brainstorming based on these. \\ Together with Yagiz explored the alternative fixed-point approximation approach using 16-bit entries and its correctness proof (the exploration stopped due to its divergence from the main approach). 

\mypar{Klejdi} Focused on cache optimizations and related metrics. Implemented the tiling logic (including WandB simulation runs) and
introduced dynamic programming subaperture contribution-count optimization (listing \ref{lst:dp-counts}). Generated a few of the plotting scripts.
