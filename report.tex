...
As described in Section \ref{subsec:vectorized-single-focus}, the increased computation SIMD capabilities of the vectorized code implemented with vector intrinsics were accompanied by tackling the data transfer stalls. The cache blocking reduced the cache misses across different levels of cache, while the loads and conversion reuse approach further reduced the performed accesses to L1D cache, since the data stayed in the registers. Besides the anticipated runtime improvement, in the experiments the Linux \texttt{perf}'s hardware counter confirmed that, as we expected, the number of loads from L1D cache goes down by around 30\% \todo{check} as depicted in Figure~\ref{fig:avxReuseL1DLoads}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{l1d_loads_normalized.pdf}
    \caption{L1D cache reduced loads, in the most optimized coed the fetched and converted data stays in the registers}
    \label{fig:avxReuseL1DLoads}
\end{figure}

With all these optimizations the most optimized code reaches around 18.5\todo{Check these numbers, no access to the data for it, @Klejdi help if possible} FLOPs/cycle which is around 70\% of the corresponding theoretical peak performance (26.67 FLOPs/cycle) for this instruction mix. Similarly, as for the scalar code, our roofline model contains different performance roofs depending on the instruction mix and conversions reuse. Additionally, the plot contains the performance of the most optimized focal stack implementation, which is discussed in the paragraph further below.
...
The code does not reach the peak performance becasue ... \todo{important question that Jonas also asked.}
