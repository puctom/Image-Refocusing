...
\begin{abstract}
Like an okapi that's almost-but-not-quite a giraffe, our first Rust implementation fell short of being well optimized. Where a giraffe stretches tall and lean, built for reach and efficiency, our implementation stayed closer to the ground: functional, recognizable, but never quite extending into the form it was meant to take. The okapi survives perfectly well in the underbrush; it simply isn't built to graze the canopy. Our code was the same. It met the requirements and never reached past them. Closing that gap meant better code patterns for cache allocation, AVX vectorization, and flop reduction. And just as a giraffe outreaches an okapi, the optimized implementation now supersedes the original. The lineage stayed the same - what changed was how far it could reach.
\end{abstract}
...
\mypar{Contribution}
We present an optimized single-core C++ implementation of light field refocusing, built up incrementally from a baseline ported directly from the provided Rust reference. Because refocusing is embarrassingly parallel (every output pixel, and every focus value, can be computed independently) much prior work scales it across many threads or offloads it to a GPU. We instead ask how much performance we can extract from a single core, where the memory hierarchy and instruction-level bottlenecks dominate, and this is exactly where our optimizations lie. To find the most suitable optimizations, we were guided by careful analysis of roofline plots and hardware performance metrics, and on this basis we applied the following. The total number of floating-point operations (flops) performed was reduced by reusing intermediate results across neighboring output pixels and by replacing repeated computation with analytical and dynamic-programming reformulations. We restructure both the computation and its memory access pattern to improve temporal locality, keeping working data
...
written once ($12HW$ bytes as 3 floats per pixel), so
$Q \approx 3SHW$ bytes since $S = 289$. This puts the operational
intensity at
\begin{equation}
I = \frac{W}{Q} \approx \frac{36\,SHW}{3\,SHW} = 12
\;\text{flops/byte}
\end{equation}
well above both ridge points of our machine ($0.70$ scalar, $5.61$
SIMD flops/byte), so the algorithm is compute-bound.

\mypar{A note on the Operational Intensity (I) estimate}
Computing $I$ exactly requires accounting for all data movement in and out of cache, not just compulsory misses: in practice, data is repeatedly evicted and re-fetched, so the true $Q$ is larger than $3SHW$ and the intensity correspondingly lower. Measuring actual traffic with Intel Advisor confirms this, yielding a value slightly below $12$ but still well above both ridge points. As will be seen later in this report, we put considerable effort into keeping our working set resident in cache for as long as possible, which pulls the measured intensity back toward this compulsory-miss ideal. So although the operational intensity above is a simplified estimate, it still serves our purpose and is close to actual empirical estimates (especially for our most optimized code). Additionally, a few of our optimizations reduce the flop count $W$; where this happens we adjust the operational intensity accordingly, as reflected in the roofline plots that follow.
...
\mypar{Adjusting the loop order} The first scalar optimization we performed was adjusting the loop order. The original loop order in the baseline implementation is shown in Figure \ref{fig:loop-order-1}; it traversed all subaperture images for a single pixel of the output image, then moved to the next output pixel on that row. The main problem with this approach is that the 64B cache block loaded from a subaperture image into the cache gets evicted before the next column from that row is read.. We fixed this issue by changing the loop order to first traverse through all pixels of a single subaperture image before moving to the next subaperture image. This loop order is shown in Figure \ref{fig:loop-order-2}. This optimization also allowed us to rewrite the mathematical computation performed in the innermost loop in a more efficient way by allowing us to compute the weight of each corner pixel contributing to the same output pixel once per subaperture image. Overall, this optimization reduced the runtime by 37.2\% compared to the baseline C++ implementation.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.2\textwidth}
        ...
        \caption{Globally Optimized Dependency Tree}
...
Having implemented all the optimizations for the scalar code, we moved on to exploiting the SIMD capabilities of the processor using vectorization. 

\mypar{Best compiler vectorized code} Before hand-crafting the vectorized code, we compiled our final, most-performant scalar code without the \texttt{-fno-tree-vectorize} flag. This gave us the compiler-vectorized code, which we evaluated in the further experiments. There was no performance gain when compared to just the scalar code. 
...
Looking globally at the algorithm, it becomes clear that for one subaperture image the adjacent output pixels and their channels can be processed in parallel. They accumulate the contributions from the neighbouring subaperture data, which is accessed in the same sequential pattern. For these reasons we opted for using explicit intrinsics to implement this approach. 

In the initial implementation, for each subaperture once the pointer to the starting pixel due to offset is determined, the input subaperture bytes are loaded: \texttt{\_mm\_loadl\_epi64} and converted to floats: \texttt{\_mm256\_cvtepi32\_ps}, and the weights are stored in their vectors \texttt{\_mm256\_set1\_ps}. Similarly, the output vector is loaded \texttt{\_mm256\_loadu\_ps} and then a chain of FMA instructions is performed \texttt{\_mm256\_fmadd\_ps}, accumulating the contributions from the corner pixels weighted by the bilinear weights. 

Such code gives a large improvement of 77\% of the runtime for small image sizes (below 1024 pixels), and about 61\% for larger image sizes. 
% Doesn't sound too drastic but is >50% slower TODO


\mypar{Blocking for cache hits on output image}
With the established \texttt{sub-y-x} loop order, the whole output image is accessed over and over again for every subaperture. However, the cache levels (as summarized in Table \ref{tab:cache_specs}) are too small to fit the output image (and the currently processed subaperture) in their entirety, which results in misses across all levels when accessing the output image, causing stalls in turn. Already for an image size of 1024x1024 pixels, the output requires: $1024 \cdot 1024 \cdot 3 \cdot 4B = 12MB $ of memory, which exceeds the 8MB of the LLC capacity.

Bearing in mind the neat structure of the algorithm with sequential pixel accesses, we decided to process a smaller tile of an output image for all subapertures at a time, making sure that accesses to that output memory for subsequent subapertures are hits, rather than misses due to not fitting in cache. An arbitrary tile size is possible, with the access pattern illustrated in \ref{fig:cacheBlocking}, achieved by using \texttt{tile\_y - tile\_x - sub - y - x} loop order.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{cacheBlocking.png}
    \caption{Blocking the output image }
    \label{fig:cacheBlocking}
\end{figure}


Initially, a size of 8 rows $ \times $ 256 columns was used, as we suspected that such tile size ($8 \cdot 256 \cdot 3 \cdot 4B = 24$KB) fitting in the L1D cache (48KB) would be optimal. 

We then tuned the tile dimensions empirically. Rather than scanning the height-width grid exhaustively, we used a Bayesian optimizer over the two parameters, which iteratively proposes tile sizes and converges on the combination that minimizes the median measured cycle count. We utilized the Weight and Biases platform  \cite{wandb} to run these simulations and kept a fixed image size of 2048x2048 when running them. The best results were achieved by the tile sizes with large width (around 2000) and small height (around 8), as show in Figure \ref{fig:wandb}. Ultimately, a tile size of 8 rows $ \times $ 2048 (full row) columns was chosen. These optimizations further reduced the proportion of misses: with the number of loads in LLC going down from 704k to 363k for 2048x2048 image size and the corresponding miss ratio down from 99\% to 68\%. 
...
\mypar{Increased ILP through loop unrolling}
On top of the code with cache-oriented tiling, we explicitly unrolled the innermost loop to process two vectors of output channels in the loop body. This way, the compiler explicitly generates more independent corresponding conversions (\texttt{vpmovzxbd}, \texttt{vcvtdq2ps}) and arithmetic (\texttt{vfmadd132ps}) instructions in the assembly. This optimization gave, however, a small speedup of about 4\% of the runtime. We suppose that since the instructions operate on independent data, the out of order execution already achieved the majority of the possible ILP.

\mypar{Load and conversion reuse} %todo add ref to scalar version
The final optimization with the vectorized code followed the same idea as the \textit{Load and conversion reuse} for scalar code. As illustrated in \ref{fig:avxReuseIllustration}, in case of the vectorized code however, the contributing subaperture pixels cannot be reused for row-adjacent output vectors, because the offset for output channels is 8, while the offset for contributing channels is 3. Fortunately, the intermediate rows of the tile can be used first as the bottom and then for the next row as the top contributing neighbours. 
By going row-by-row within a tile, we reuse the intermediate loaded and converted rows. Except for the outer pixels, this is applied in blocks of 4 rows, leading to reduction of the loads and conversions from 8 (2 per row) to 5 (reusing 3 intermediate rows). This is confirmed by reduced L1D cache loads, since the data stays in the registers and does not need to be fetched from the cache. The optimization improves the runtime by further \todo{\%}, while the FLOPs/cycle stays at roughly the same level, with the FLOPs count reduced to: \todo{}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{avxReuseIllustration.png}
    \caption{Avx Reuse Illustration}
    \label{fig:avxReuseIllustration}
\end{figure}
\todo{improve the illustration if it is to stay here}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{avxReuseL1DLoads.png}
    \caption{L1D cache reduced loads, data stays in the registers}
    \label{fig:placeholder}
\end{figure}
\todo{should this go here, later in experiments, not at all?}
...
The aforementioned processor has 3 levels of cache, an L1 data cache of 48KB per core, an L2 cache of 1.25MB per core, and a shared LLC of 8MB.

In all our measurements, we used cold-caches and disabled frequency-scaling, which makes the processor run at 2.4 GHz. 

Our experimental setup had 8GB of RAM with a peak memory bandwidth of 5.7 Bytes/Cycle, measured by the STREAM~\cite{McCalpin:95} benchmark.

We always used the \textbf{-O3}, \textbf{-march=native} and \textbf{-ffast-math} flags when compiling our code. Our scalar optimizations also used the \textbf{-fno-tree-vectorize} flag to explicitly disable the compiler from vectorizing.

We used square RGB images ranging in height/width from 256 up to 4096. In our performance plots, the performance drops heavily for images of size 3072x3072 and above. This is because the 289 subaperture images take up more than 8GB of space for these image sizes. Since this does not fit in the RAM, it forces the program to use the swap space for storing subaperture images, which incurs disk I/O latency and throughput penalties when accessing their data. As a result, our discussion below will focus on images of size 2048x2048 and below.
...
Figure \ref{fig:avxCyclesPerPixelResults} plots the performance for the AVX2 versions of the code. AVX2 brings the runtime from the previous best scalar $13.9$~Gcycles down to $2.0$~Gcycles, a further $7.0\times$ speedup, which is a total $62.5\times$ speedup over the original baseline. The best vectorized single focus code reaches ... \todo flops/cycle, ...\% of the scalar peak. The speedup is close to but slightly below the expected $8\times$ speedup from the lane width. The IPC drops from 1.18 to 0.78 because the byte to float conversions use the same ports for the vector instructions as the vector FMAs. This is also why we cannot reach the SIMD roof.
